...
const AdminCouponList = () => {
    ...
    const fetchCoupons = async () => {
        try {
            ...
            // Normalize data to handle schema mismatch
            const normalizedData = (data || []).map(item => ({
                ...
                discount_type: item.discount_type, // Keep 'percent' as is
                usage_count: item.usage_count ?? 0,
                usage_limit: item.usage_limit ?? null
            }));

            // console.log('Coupons fetched (normalized):', normalizedData);
            setCoupons(normalizedData);
        } catch (error) {
            console.error('Error fetching coupons:', error);
            // Don't show error toast on initial empty load if table doesn't exist yet, 
            // but assuming schema exists from previous steps.
