export interface Producto {
  ...
  precio: number;
  categoria?: string;
  imagen?: string | null; // ✅ se agregó "null"
  codigo?: string;
  descripcion_adicional?: string;
}

// Función para asegurar que todos los productos tengan valores seguros
export const normalizarProducto = (p: Partial<Producto>): Producto => ({
  id: p.id ?? 0,
  nombre: p.nombre ?? "Producto sin nombre",
  subtitulo: p.subtitulo ?? "Sin subtítulo disponible",
  descripcion: p.descripcion ?? "Descripción no disponible",
  tamaño_ml: p.tamaño_ml ?? "No indicado",
  tamaño_fl_oz: p.tamaño_fl_oz ?? "No indicado",
  precio: p.precio ?? 0,
  categoria: p.categoria ?? "Sin categoría",
  imagen: p.imagen ?? "https://via.placeholder.com/300x300?text=Perla+Negra",
  codigo: p.codigo ?? "N/D",
  descripcion_adicional: p.descripcion_adicional ?? "Información adicional no disponible",
});
...
] as Partial<Producto>[]).map(normalizarProducto);
